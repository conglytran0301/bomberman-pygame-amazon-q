...
class GameController:
    ...
    def show_difficulty_selection(self, in_game=False):
        ...
        if in_game:
            ...
            # Calculate optimal screen size (80% of available screen)
    ...
    def start_game(self, difficulty):
        ...
        # Calculate optimal tile size based on screen resolution
        max_grid_width = int(self.screen_width * 0.9)  # Use 90% of screen width
        max_grid_height = int(self.screen_height * 0.85)  # Use 85% of screen height for grid (leave room for UI)
        
        # Calculate tile size that fits the grid
        tile_size_width = max_grid_width // self.grid_size
        tile_size_height = max_grid_height // self.grid_size
        TILE_SIZE = min(tile_size_width, tile_size_height)  # Use the smaller dimension
        ...
        screen_height = self.grid_size * TILE_SIZE + 60  # Extra space for UI
    ...
    def create_enemies(self):
        ...
        for _ in range(self.difficulty["enemies"]):
            ...
            self.enemies.append(Enemy(x, y, self.assets.enemy_img))
    ...
    def update(self):
        ...
        for enemy in self.enemies[:]:
            # Move randomly with a certain probability
    ...
    def explode_bomb(self, bomb):
        ...
        self.explosions.append(Explosion(bomb.x, bomb.y, self.assets.explosion_img))
        ...
        for dx, dy in directions:
            for i in range(1, explosion_range + 1):
                ...
                if self.grid[y][x] == DESTRUCTIBLE:
                    ...
                    self.explosions.append(Explosion(x, y, self.assets.explosion_img))
                    break
    ...
    def render_ui(self):
        ...
        self.screen.blit(enemies_text, (450, self.grid_size * TILE_SIZE + 15))
        ...
        diff_text = self.small_font.render(f"Difficulty: {diff_name} (Ctrl+D to change)", True, WHITE)
        self.screen.blit(diff_text, (self.grid_size * TILE_SIZE - 200, self.grid_size * TILE_SIZE + 40))
